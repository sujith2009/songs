...
const firstsong = () => {
  ...
  const [songsFilter, setSongFilter] = useState<string>("");
    ...
    const fetchSongs = async () => {
      try {
        const response = await fetch(
          "https://41a9-49-37-209-109.ngrok-free.app/api/song"
        );
        const json = await response.json();

        if (Array.isArray(json.song)) {
          const filteredSongs = json.song
            .filter((song: Song) => song.id >= 1 && song.id <= 123)
            .map((song: Song) => ({
              ...song,
  ...
  const cleanLyrics = (lyrics: string) => {
    return lyrics
      .replace(/[\[\]0-9\\n()]/g, "") // Remove brackets, numbers, newlines, and parentheses
      .replace(/\\/g, "") // Remove any remaining backslashes
      .trim(); // Trim any leading or trailing whitespace
  };
  const threeIconsClick = (songId: number) => {
    setSelectedSongId(songId === selectedSongId ? null : songId); // Toggle selection
  };
  useEffect(() => {
    registerApp();
  }, []);
  const registerApp = async () => {
    ...
    const token = (
      await Notifications.getExpoPushTokenAsync({
        projectId: "bbd2ca84-ff53-44a1-8459-e92226295f0b",
      })
    ).data;
  ...
  const handleShareSong = async (song: Song) => {
    Alert.alert("Success to notification");

    await Notifications.scheduleNotificationAsync({
      content: {
        title: `Now playing: ${song.song_title}`,
        body: "This is a notification from irai aligal",
        data: { songId: song.id },
      },
      trigger: { seconds: 1 },
    });
  ...
  useEffect(() => {
    const subscription = Notifications.addNotificationResponseReceivedListener(
      (response) => {
        ...
        // Navigate to the song play screen with the song ID
        if (typeof songId === "string") {
          router.push(`/firstsong`);
        } else {
          Alert.alert("Error", "Song ID not found in notification.");
        }
      }
    );
    ...
    return (
      <>
        <View style={styles.songsContainer}>
          ...
          {/*--SHARE LYRICS AND PUSH NOTIFICATIONS--*/}
          <Pressable onPress={() => threeIconsClick(item.id)}>
            <MaterialCommunityIcons
              name="dots-vertical"
              size={24}
              color="black"
        ...
        {selectedSongId === item.id && (
          <View style={styles.lyricsShowDotsIcons}>
            <Pressable onPress={() => handleShareSong(item)}>
              <MaterialCommunityIcons
                name="share-variant"
                size={24}
                color="red"
  ...
  const handleFilterSongs = song.filter((fil) =>
    fil.song_title.charAt(0).toLowerCase().includes(songsFilter.toLowerCase())
  );
    ...
    <View style={styles.container}>
      <Pressable>
        <Pressable style={styles.inputContainer}>
          ...
          <TextInput
            placeholder="Search your song only Tamil"
            onChangeText={(text) => setSongFilter(text)}
            value={songsFilter}
          />
      ...
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
        ...
        ) : handleFilterSongs.length > 0 ? (
          <FlatList
            style={{ marginTop: 10 }}
            data={handleFilterSongs}
            ...
            >
              <View style={styles.modalView}>
                ...
                <ScrollView style={styles.lyricsContainer}>
                  <Text>
                    {cleanLyrics(currentSong?.song_description || "")}
                  </Text>
                </ScrollView>
              </View>
            </Modal>
